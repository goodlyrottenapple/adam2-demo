...
async def getOntologyInternal(payload:Query, checksumCurrent = None):
  ...
  if path.exists(cached_path) and checksumCurrent is None:
    ...
    #         _,l = r[0]
    #         res[o_str] = l
    #   res = [{"value": k, "label":l} for (k,l) in res.items()]
...
async def getOntologyFlat(payload:Query):
  '''
  Fetch new ontology which doesnt have subclassOf
  '''
  cached_path = "ontologies/"+slugify(payload.url)+'-flat.json'
...
async def getOntology(payload:Query):
  ...
  try:
    if payload.collapseTree:
      return await getOntologyFlat(payload)
    _, res = await getOntologyInternal(payload)
    if len(res) == 0:
      res = await getOntologyFlat(payload)
    return res
  except Exception as e:
